...
1. **Urna Eletrônica (Hardware Isolado)**:
    ...
    - **Auditoria de Última Instância**: A Blockchain fornece a integridade global, mas o voto de papel é o "Voto Soberano". Em caso de auditoria por amostragem ou contestação, a contagem manual do papel deve bater com o registro digital. Qualquer divergência anula a seção.
    - **Recibo de Auditoria do Eleitor**: A Urna exibe um **Hash de Rastreio Anônimo** (derivado do Token). O eleitor pode usá-lo para consultar o portal e verificar se seu voto está na Blockchain. 
...
2. **Centro de Processamento e Transparência Radical**:
    - **Distribuição Multi-Entidade**: Cópias idênticas da Blockchain de cada urna são enviadas para o TSE e para entidades cadastradas (OAB, Universidades, Partidos). 
    - **Imutabilidade Distribuída**: Como as entidades possuem cópias, o TSE não pode alterar a Blockchain central sem ser desmascarado pelo Hash do Boletim de Urna impresso e pelas cópias em posse da sociedade civil.
    - **Totalizador (Java)**: Consolida os votos. Qualquer cidadão pode usar o Hash impresso na seção para conferir se a Blockchain da sua urna foi processada corretamente no portal.

## User Review Required

> [!IMPORTANT]
> **Foco no TSE Malicioso (Remoção do Banco Decoy):**
> Conforme sugerido, eliminamos o mecanismo de "Banco Decoy" para simplificar a arquitetura e remover o risco de o TSE trocar os bancos de dados. A segurança agora reside 100% na **Auditabilidade Pública da Blockchain**. Se o TSE tentar inserir votos falsos ou remover votos legítimos, a matemática da Blockchain (assinada com chaves que o TSE não possui) provará a fraude instantaneamente para qualquer auditor independente.
...
- **Mitigação:** 
    ...
    - **Root of Trust Descentralizada e Resiliência**: 
        1. O Registro Público de Chaves é uma **Blockchain Distribuída**. Além do TSE e entidades, **qualquer cidadão pode baixar uma cópia completa** desse registro. Isso permite que cada eleitor verifique se sua chave pública está correta e se não houve inserção de chaves falsas.
        ...
        4. **Prova de Fraude**: Se o TSE publicar um resultado diferente do que os scripts independentes calcularem, a **Blockchain Pública serve como prova irrefutável da fraude**. Como os dados são públicos, qualquer tribunal (ou a própria sociedade) pode reprocessar a soma e expor a mentira do administrador.
...
2.  **"O TSE pode apagar votos da base de dados":**
    *   *Resposta:* Os dados estão em uma **Blockchain Distribuída**. Para apagar um voto, o TSE teria que apagar simultaneamente as cópias em posse da OAB, Universidades, Partidos e dos próprios cidadãos. É impossível deletar a história quando ela está em milhares de mãos ao mesmo tempo.
3.  **"Eu quero ver meu voto no papel":**
    *   *Resposta:* O seu **Hash de Rastreio** e o seu **Token** são sua "cédula digital". Diferente do papel, que pode ser extraviado ou queimado no transporte, o seu rastreio está "escrito na pedra" da Blockchain pública. Você pode conferir se ele foi contado a qualquer hora, de qualquer lugar, sem depender de ninguém.
...
- **Blockchain Engine**: Implementação de Merkle Trees para garantir que nenhum voto seja alterado sem quebrar toda a corrente.
...
- Um pequeno utilitário (CLI ou Web) para que o cidadão valide a Blockchain baixada em sua própria máquina.

## Verification Plan

1. **Teste de Integridade**: Tentar alterar um voto no arquivo da Blockchain e verificar se o Totalizador Java detecta a quebra do Hash.
